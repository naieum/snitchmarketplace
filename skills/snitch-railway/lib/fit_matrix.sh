...
run_fit_matrix() {
  ...
  if [[ -z "$entry" ]]; then
    printf '{"error":"unknown stack","code":"E_UNKNOWN_STACK","stack":"%s","remediation":"run snitch-railway.sh fit-matrix without args to see all stack keys"}\n' "$stack" >&2
    return 5
  fi
