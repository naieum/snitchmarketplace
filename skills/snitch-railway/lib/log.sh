...
RWSEC_FINDINGS_FILE="${STATE_DIR:-/tmp}/findings.tsv"
: > "$RWSEC_FINDINGS_FILE" 2>/dev/null || true
...
log_ok() {
  ...
  [[ -n "$url" ]] && printf ' → %s' "$url"
  printf '\n'
  printf 'OK\t%s\t%s\t%s\t%s\n' "$area" "$key" "$msg" "$url" >> "$RWSEC_FINDINGS_FILE" 2>/dev/null || true
}
...
log_locked() {
  ...
  [[ -n "$url" ]] && printf ' → %s' "$url"
  printf '\n'
  printf 'LOCKED\t%s\t%s\t%s\t%s\t%s\n' "$area" "$key" "$msg" "$tier" "$url" >> "$RWSEC_FINDINGS_FILE" 2>/dev/null || true
}
...
findings_count() {
  local kind="$1"
  [[ -f "$RWSEC_FINDINGS_FILE" ]] || { printf '0'; return; }
  awk -F'\t' -v k="$kind" '$1==k{n++} END{print n+0}' "$RWSEC_FINDINGS_FILE"
}
...
snapshot_write() {
  ...
  cp "$RWSEC_FINDINGS_FILE" "$out" 2>/dev/null || true
