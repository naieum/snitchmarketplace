...
# against each in-scope hostname.
# Exports: run_score
...
_score_security_headers() {
  ...
  curl -sS -A "snitch-railway-skill" -o "$cache" \
    "https://securityheaders.com/?q=${host}&followRedirects=on&hide=on" \
    >/dev/null 2>&1 || true
  if [[ ! -s "$cache" ]]; then
    log_warn "score" "secheaders/${host}" "no response body — skipping"
    return 0
  fi
  local grade
  ...
  if [[ -z "$grade" ]]; then
    log_warn "score" "secheaders/${host}" "could not parse grade — skipping"
    return 0
  fi
...
run_score() {
  ...
  if [[ -z "$hosts" ]]; then
    log_warn "score" "hosts" "no hostnames in scope. Pass them as args or write to ${STATE_DIR}/hostnames.txt."
    return 0
  fi
