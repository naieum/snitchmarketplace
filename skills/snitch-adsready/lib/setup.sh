...
_setup_steps_structured_data() {
  jq -n '
    [
      { id: "precheck",
        kind: "auto",
        title: "Inspect existing JSON-LD",
        description: "Parse <script type=\"application/ld+json\"> blocks; report what types (Organization, WebSite, BreadcrumbList, Product, Article, FAQ) are present.",
        verify_command: "bash ads-ready.sh state site <url> structured-data" },
      { id: "identify-vertical",
        kind: "manual",
        title: "Pick the vertical(s) that apply",
        description: "Ecommerce → Product, Article. SaaS → Organization, WebSite, FAQ. Marketing-site → Organization, WebSite. Blog → Article, BreadcrumbList. Multi-vertical → multiple types are fine on the same page where they describe distinct entities." },
      { id: "apply-base",
        kind: "auto",
        title: "Apply Organization + WebSite + BreadcrumbList JSON-LD",
        description: "Renders templates/structured-data/{organization,website,breadcrumb}.starter.json with placeholders substituted from the user-supplied org details (name, url, logo, sameAs).",
        fix_command: "bash ads-ready.sh fix structured-data" },
      { id: "apply-vertical",
        kind: "auto",
        title: "Apply vertical-specific schemas",
        description: "Per detected vertical, also emit Product / Article / FAQ starter JSON-LD on the relevant pages.",
        fix_command: "bash ads-ready.sh fix structured-data" },
      { id: "validate",
        kind: "external-tool",
        title: "Validate with Google Rich Results Test + Schema.org Validator",
        description: "After deploy, paste the page URL into the validators. Resolve every error. Warnings are usually optional fields; resolve high-value ones (image, aggregateRating).",
        external_url: "https://search.google.com/test/rich-results" },
      { id: "verify-state",
        kind: "auto",
        title: "Re-run state site to confirm coverage",
        description: "structured-data slice should report all expected types present.",
        verify_command: "bash ads-ready.sh state site <url> structured-data" }
    ]
  '
}
  ...
  jq -n '
    [
      ...
      { id: "apply-headers",
        ...
        description: "Renders templates/security-headers-for-ads.template.txt for the detected stack (next.config.js headers(), _headers for Pages, vercel.json headers, .htaccess for WP / Apache, etc.). Includes CSP that allowlists every platform script + iframe + img domain.",
  ...
  jq -n '
    [
      ...
      { id: "list-crawlers",
        ...
        description: "Ad platforms verify landing pages with their own crawlers (AdsBot-Google / AdsBot-Google-Mobile, bingbot + AdIdxBot, facebookexternalhit, LinkedInBot, Bytespider, …). A Disallow that catches one of them fails ad review or hurts Quality Score. Also decide the AI-crawler policy (GPTBot, OAI-SearchBot, PerplexityBot) — blocking them removes the site from AI-search surfaces including ChatGPT.",
