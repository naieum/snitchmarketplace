...
#
# Output rules:
#   - One finding per line, prefixed with the status badge (with emoji).
#   - Append docs URL when relevant ("→ <url>").
#   - Append capability when feature is locked ("[locked: <capability>]").
...
log_ok() {
  ...
  [[ -n "$url" ]] && printf ' → %s' "$url"
  printf '\n'
  printf 'OK\t%s\t%s\t%s\t%s\n' "$area" "$key" "$msg" "$url" >> "$ADSEC_FINDINGS_FILE" 2>/dev/null || true
}
...
# Counters used at end of pass to set exit code.
findings_count() {
  local kind="$1"
  [[ -f "$ADSEC_FINDINGS_FILE" ]] || { printf '0'; return; }
  awk -F'\t' -v k="$kind" '$1==k{n++} END{print n+0}' "$ADSEC_FINDINGS_FILE"
}
...
snapshot_write() {
  ...
  cp "$ADSEC_FINDINGS_FILE" "$out" 2>/dev/null || true
