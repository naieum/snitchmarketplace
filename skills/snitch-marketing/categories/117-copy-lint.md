...
- "The copy is vague." Quote the line and name which pattern it trips. An adjective about the copy is not a finding about the copy.
- "This claim is unsupported." Check the page and the pages it links to first; a claim substantiated one click away is substantiated.
- "This is a dark pattern." Run the boundary test in Cat 114 before using the term; pressure is not deception.
- "Nobody would convert on this." This category audits the line against the pattern table, never against an imagined visitor's reaction.
- Rewriting the brand's voice into your own. A voice-constant violation is a finding only when the project's own documented voice says so (`references/brand-voice-framework.md`).
...
- **Cat 115 (Pricing psychology tactical)** — "starting at" with no number routes here; how the tiers are displayed routes to Cat 115, and the strategic question of what to charge is generated by calling the Skill tool with "snitch-cmo".
- **Cat 60 (Conversion & trust)** — anonymous social proof is a Cat 60 trust-artifact gap; this cat catches the specific copy line that needs replacement.
...
Cross-reference: `references/brand-voice-framework.md` when the copy finding turns on the brand's documented voice (a voice-constant violation) rather than on a vague or unsupported claim.

### Fix voice

`honest-design-critic` (primary) | `plain-language-designer` (backup).

The honest-critic voice for "you're telling the buyer nothing, and you're charging them for the privilege of reading it." The plain-language voice as backup when the fix is "rewrite this hero so it says what the thing actually does, in one breath."
...
The patterns below were validated against a body of landing-page teardowns by copywriters who test what converts, not against generic SEO writing guides. The audit cites the specific copy line, not "the brand should write better" generic advice.
