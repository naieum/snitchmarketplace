...
>
> For e-commerce collection / category pages, the fix is the flip side of the finding: these are the site's natural landing pages for head terms ("claddagh ring", not "claddagh ring meaning"), and a bare product grid concedes that ranking. Give the collection real content — what unites these products, how to choose between them, the vocabulary buyers actually use — written only from facts and wording already on the merchant's own site (no invented claims; cross-ref `references/anti-hallucination.md`), plus CollectionPage/ItemList schema. On platforms with rule-based collections (Shopify smart collections: "product title contains X"), one rule plus one page of curation copy turns an unranked grid into the site's strongest head-term asset, and it stays current as products change. Keep the copy from stealing the grid's place: above the fold belongs to products, curation prose sits below or collapsed.
