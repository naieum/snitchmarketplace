...
#   - Append tier when feature is locked ("[locked: scale+]").
#   - log_section prints a single H2-ish separator.
#   - Stderr is for human chatter; stdout is the report.

FLYSEC_FINDINGS_FILE="${STATE_DIR:-/tmp}/findings.tsv"
: > "$FLYSEC_FINDINGS_FILE" 2>/dev/null || true
...
log_ok() {
  ...
  [[ -n "$url" ]] && printf ' → %s' "$url"
  printf '\n'
  printf 'OK\t%s\t%s\t%s\t%s\n' "$area" "$key" "$msg" "$url" >> "$FLYSEC_FINDINGS_FILE" 2>/dev/null || true
}
...
log_locked() {
  ...
  [[ -n "$url" ]] && printf ' → %s' "$url"
  printf '\n'
  printf 'LOCKED\t%s\t%s\t%s\t%s\t%s\n' "$area" "$key" "$msg" "$tier" "$url" >> "$FLYSEC_FINDINGS_FILE" 2>/dev/null || true
}
...
findings_count() {
  local kind="$1"
  [[ -f "$FLYSEC_FINDINGS_FILE" ]] || { printf '0'; return; }
  awk -F'\t' -v k="$kind" '$1==k{n++} END{print n+0}' "$FLYSEC_FINDINGS_FILE"
}

snapshot_write() {
  local ts; ts=$(date -u +%Y%m%dT%H%M%SZ)
  local out="${STATE_DIR}/snapshot-${ts}.tsv"
  cp "$FLYSEC_FINDINGS_FILE" "$out" 2>/dev/null || true
