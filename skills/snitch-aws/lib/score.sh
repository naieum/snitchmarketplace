# lib/score.sh — external validator scoring (read-only, offline-friendly).
# Runs SSL Labs, Mozilla Observatory, HSTS Preload, securityheaders.com against
# each in-scope hostname (CloudFront / ALB / S3-website / API GW custom domain).
# Caches results where the upstream supports it.
#
...
_score_ssllabs() {
  ...
  jq -r '.endpoints[]? | "\(.ipAddress)\t\(.grade // .statusMessage // "?")"' "$cache" 2>/dev/null \
    | while IFS=$'\t' read -r ip grade; do
...
_score_observatory() {
  ...
  curl -sS -o "$cache" \
    -X POST "https://http-observatory.security.mozilla.org/api/v1/analyze?host=${host}" \
    >/dev/null 2>&1 || true
  local i state
...
_score_security_headers() {
  ...
  curl -sS -A "snitch-aws-skill" -o "$cache" \
    "https://securityheaders.com/?q=${host}&followRedirects=on&hide=on" \
    >/dev/null 2>&1 || true
  if [[ ! -s "$cache" ]]; then
    log_warn "score" "secheaders/${host}" "no response body — skipping rather than guess a grade"
    return 0
  fi
  local grade
  ...
  if [[ -z "$grade" ]]; then
    log_warn "score" "secheaders/${host}" "could not parse grade from HTML — skipping rather than fake one"
    return 0
  fi
