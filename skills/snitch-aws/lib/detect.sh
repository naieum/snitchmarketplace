...
# detection plus AWS-specific signals (IaC frameworks, ECR push hints, profiles).
#
# Exports:
...
_det_iac() {
  ...
  if compgen -G "*.tf" >/dev/null 2>&1 || compgen -G "*.tf.json" >/dev/null 2>&1; then
    out+=("terraform")
    # Detect AWS provider blocks
    if grep -r -E -l 'provider[[:space:]]+"aws"' --include='*.tf' --include='*.tf.json' . 2>/dev/null | head -n1 | grep -q .; then
      out+=("terraform-aws-provider")
    fi
  fi
  # CDK
  [[ -f "cdk.json" ]] && out+=("cdk")
  ...
  if [[ -f "template.yaml" || -f "template.yml" ]]; then
    if grep -E -q '^Transform:[[:space:]]*AWS::Serverless' template.yaml 2>/dev/null \
       || grep -E -q '^Transform:[[:space:]]*AWS::Serverless' template.yml 2>/dev/null; then
      out+=("sam")
    else
      out+=("cloudformation")
    fi
  fi
  [[ -f "samconfig.toml" ]] && out+=("sam")
  # Serverless Framework
  [[ -f "serverless.yml" || -f "serverless.yaml" || -f "serverless.ts" || -f "serverless.js" ]] && out+=("serverless-framework")
  # Pulumi
  [[ -f "Pulumi.yaml" ]] && out+=("pulumi")
  if compgen -G "Pulumi.*.yaml" >/dev/null 2>&1; then
    out+=("pulumi")
    if compgen -G "Pulumi.aws*.yaml" >/dev/null 2>&1; then
      out+=("pulumi-aws")
    fi
  fi
  # AWS Copilot
  [[ -d "copilot" ]] && out+=("copilot")
  # AWS CDK packages
  if [[ -f "package.json" ]]; then
    [[ "$(_det_pkg_has '"aws-cdk-lib"')" == "1" ]] && out+=("cdk")
    [[ "$(_det_pkg_has '"@aws-cdk/')" == "1" ]] && out+=("cdk")
  fi
...
_det_aws_profiles() {
  ...
  if [[ -f "$HOME/.aws/credentials" ]]; then
    while IFS= read -r line; do
      [[ -n "$line" ]] && out+=("$line")
    done < <(grep -E '^\[' "$HOME/.aws/credentials" 2>/dev/null | sed -E 's/^\[(.*)\]$/\1/')
  fi
  if [[ -f "$HOME/.aws/config" ]]; then
    while IFS= read -r line; do
      [[ -n "$line" ]] && out+=("$line")
    done < <(grep -E '^\[' "$HOME/.aws/config" 2>/dev/null | sed -E 's/^\[(profile[[:space:]]+)?(.*)\]$/\2/')
  fi
  printf '%s' "$(_det_dedupe_to_json "${out[@]+"${out[@]}"}")"
}

_det_aws_sso_profiles() {
  local has=0
  if [[ -f "$HOME/.aws/config" ]] && grep -E -q '^[[:space:]]*sso_(start_url|session)' "$HOME/.aws/config" 2>/dev/null; then
    has=1
  fi
  if [[ "$has" == "1" ]]; then printf 'true'; else printf 'false'; fi
}

_det_ecr_pushes() {
  local has=0
  if [[ -f "Dockerfile" ]]; then
    if grep -r -E -l 'amazonaws\.com|ecr-public' --include='*.yml' --include='*.yaml' --include='Dockerfile' --include='*.sh' . 2>/dev/null | head -n1 | grep -q .; then
      has=1
    fi
  fi
  if grep -r -E -q 'aws ecr (get-login-password|create-repository)' --include='*.yml' --include='*.yaml' --include='*.sh' . 2>/dev/null; then
    has=1
  fi
  if [[ "$has" == "1" ]]; then printf 'true'; else printf 'false'; fi
}
...
_det_stacks() {
  ...
  # Lambda native: aws-lambda-* libs in package.json or python lambda handler.
  if [[ "$(_det_pkg_has '"aws-lambda"')" == "1" || "$(_det_pkg_has '"@aws-sdk/client-lambda"')" == "1" ]]; then
    out+=("lambda-native")
  fi
...
_det_databases() {
  ...
  # DynamoDB hints
  if grep -r -E -q '@aws-sdk/client-dynamodb|boto3.*dynamodb|DynamoDBClient' \
...
_det_ai_providers() {
  ...
  [[ "$(_det_pkg_has '"@aws-sdk/client-bedrock-runtime"')" == "1" ]] && out+=("aws-bedrock")
...
_det_current_host_provider() {
  ...
  if [[ -f "amplify.yml" || -d "amplify" ]]; then printf '"aws-amplify"'; return; fi
  if [[ -f "apprunner.yaml" ]]; then printf '"aws-apprunner"'; return; fi
  if [[ -f ".elasticbeanstalk/config.yml" ]]; then printf '"aws-elasticbeanstalk"'; return; fi
  if [[ -f "cdk.json" || -f "samconfig.toml" ]]; then printf '"aws"'; return; fi
  if [[ -f "serverless.yml" || -f "serverless.yaml" ]]; then printf '"aws-serverless-framework"'; return; fi
  if compgen -G "*.tf" >/dev/null 2>&1 \
    && grep -r -E -l 'provider[[:space:]]+"aws"' --include='*.tf' . 2>/dev/null | head -n1 | grep -q .; then
    printf '"aws"'; return
  fi
  printf 'null'
}
...
_det_hostnames() {
  ...
  # Try CDK / SAM / serverless for custom domain hints.
  if compgen -G "*.tf" >/dev/null 2>&1; then
    while IFS= read -r line; do [[ -n "$line" ]] && out+=("$line"); done < <(grep -r -E -h 'domain_name[[:space:]]*=[[:space:]]*"[^"]+"' --include='*.tf' . 2>/dev/null \
      | sed -E 's/.*"([^"]+)".*/\1/' | sort -u)
  fi
  if [[ -f "samconfig.toml" || -f "template.yaml" || -f "template.yml" ]]; then
    while IFS= read -r line; do [[ -n "$line" ]] && out+=("$line"); done < <(grep -r -E -h 'DomainName[[:space:]]*:[[:space:]]*[^"]*"[^"]+"' --include='*.yaml' --include='*.yml' --include='*.toml' . 2>/dev/null \
      | sed -E 's/.*"([^"]+)".*/\1/' | sort -u)
  fi
  printf '%s' "$(_det_dedupe_to_json "${out[@]+"${out[@]}"}")"
}
...
run_detect() {
  ...
  local stacks dbs storage native bgworkers ws ai_providers vector_dbs embed headless project_kind pms host_provider hostnames iac profiles sso ecr
  ...
  iac="$(_det_iac)"
  profiles="$(_det_aws_profiles)"
  sso="$(_det_aws_sso_profiles)"
  ecr="$(_det_ecr_pushes)"
  ...
  jq -n \
    ...
    --argjson iac            "$iac" \
    --argjson aws_profiles   "$profiles" \
    --argjson aws_sso_available "$sso" \
    --argjson ecr_pushes     "$ecr" \
    '{
      schema: "awssec.detect",
      schema_version: 1,
      generated_at: $ts,
      tool: "detect",
      cwd: $cwd,
      ...
      aws_sso_available: $aws_sso_available,
