...
Target bands: strict <= 1.5/100w, flavored <= 2.5/100w, banned words zero in both.

Usage:
  python3 ste-lint.py file.md [more files or globs]   # per-file summary table
  python3 ste-lint.py < draft.txt                     # JSON detail on stdin

Code fences and inline code spans are stripped before scoring — code is never linted.
"""
...
def lint(text):
    ...
    v["em_dash"] = strip_code(raw).count("—") + strip_code(raw).count("–")
