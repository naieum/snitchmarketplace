...
# SSL Labs, Mozilla Observatory, HSTS Preload, securityheaders.com.
#
# Exports: run_score
...
_score_observatory() {
  ...
  [[ -s "$cache" ]] || { log_warn "score" "observatory/${host}" "no body"; return 0; }
  local grade score
...
_score_hsts_preload() {
  ...
  curl -sS -o "$cache" \
    ...
    >/dev/null 2>&1 || true
  [[ -s "$cache" ]] || { log_warn "score" "hsts-preload/${host}" "no body"; return 0; }
  local status; status="$(jq -r '.status // "?"' "$cache" 2>/dev/null)"
...
_score_security_headers() {
  ...
  curl -sS -A "snitch-digitalocean-skill" -o "$cache" \
    "https://securityheaders.com/?q=${host}&followRedirects=on&hide=on" \
    >/dev/null 2>&1 || true
  [[ -s "$cache" ]] || { log_warn "score" "secheaders/${host}" "no body"; return 0; }
  local grade
