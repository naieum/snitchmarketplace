...
run_fit_matrix() {
  ...
  if [[ -z "$entry" ]]; then
    printf '{"error":"unknown stack","code":"E_UNKNOWN_STACK","stack":"%s","remediation":"run snitch-vercel.sh fit-matrix without args to see all stack keys"}\n' "$stack" >&2
    return 5
  fi

  jq -n --arg ts "$ts" --arg stack "$stack" --argjson entry "$entry" \
    '{
      schema: "vrcsec.fit-matrix-entry",
      schema_version: 1,
      generated_at: $ts,
      tool: "fit-matrix",
      stack: $stack,
