# lib/score.sh — external validator scoring.
# Runs SSL Labs, Mozilla Observatory, HSTS Preload, securityheaders.com.
# Exports: run_score
...
_score_ssllabs() {
  ...
  if _score_cache_fresh "$cache" 3600; then
    ...
    curl -sS -o "$cache.tmp" -X POST "https://api.ssllabs.com/api/v3/analyze?host=${host}&startNew=on&all=done" >/dev/null 2>&1 || true
    local i status
    for i in $(seq 1 40); do
      sleep 6
      curl -sS -o "$cache.tmp" "https://api.ssllabs.com/api/v3/analyze?host=${host}&all=done" >/dev/null 2>&1 || true
  ...
  if [[ "$status" != "READY" ]]; then
    log_warn "score" "ssllabs/${host}" "ssllabs status=${status}"
    return 0
  fi
  local grades
...
_score_observatory() {
  ...
  curl -sS -o "$cache" -X POST "https://http-observatory.security.mozilla.org/api/v1/analyze?host=${host}" >/dev/null 2>&1 || true
  local i state
  for i in $(seq 1 30); do
    sleep 4
    curl -sS -o "$cache" "https://http-observatory.security.mozilla.org/api/v1/analyze?host=${host}" >/dev/null 2>&1 || true
    state="$(jq -r '.state // ""' "$cache" 2>/dev/null)"
    [[ "$state" == "FINISHED" || "$state" == "FAILED" || "$state" == "ABORTED" ]] && break
  done
  if [[ ! -s "$cache" ]]; then
    log_warn "score" "observatory/${host}" "observatory returned no body"
    return 0
  fi
  local grade score
...
_score_hsts_preload() {
  ...
  if [[ ! -s "$cache" ]]; then
    log_warn "score" "hsts-preload/${host}" "hstspreload returned no body"
    return 0
  fi
  local status; status="$(jq -r '.status // "?"' "$cache" 2>/dev/null)"
...
_score_security_headers() {
  ...
  curl -sS -A "snitch-azure-skill" -o "$cache" "https://securityheaders.com/?q=${host}&followRedirects=on&hide=on" >/dev/null 2>&1 || true
  if [[ ! -s "$cache" ]]; then
    log_warn "score" "secheaders/${host}" "no response body"
    return 0
  fi
  local grade
  grade="$(grep -E -o 'class="score_lite[A-F][+-]?"' "$cache" 2>/dev/null \
    | head -n 1 \
    | sed -E 's/.*score_lite([A-F][+-]?).*/\1/')"
  if [[ -z "$grade" ]]; then
