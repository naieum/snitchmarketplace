...
A fictional example (tool: "Ledgerly," a bookkeeping SaaS) showing the full flow from
`SKILL.md`: map the current page, diagnose it against CLOSER, verify claims, then reorder.

## The page as it stood

Section order, top to bottom:

1. **Hero.** "Ledgerly: real-time double-entry bookkeeping powered by our custom
   reconciliation engine." Price not mentioned.
2. **How it works.** A three-step technical diagram: bank feed → matching engine → ledger.
3. **Feature grid.** Twelve feature tiles (multi-currency, audit trail, API, etc).
4. **Pricing table.** Three tiers, $19/$49/$99.
5. **FAQ.** Three questions, none about price, cancellation, or data ownership.
6. **CTA band.** "Start your free trial." No supporting copy.

## Stage map

| Section | Stage | Notes |
|---|---|---|
| Hero | C (partial) | States what it is, but leads with the mechanism ("reconciliation engine"), not the outcome. No price, no "why am I here" hook. |
| How it works | S, mechanism half only | This is the "plane ride." Nothing on the page has sold the "vacation" yet, so it lands as noise. |
| Feature grid | S, mechanism half only | Same problem — twelve features with no outcome framing above them. |
| Pricing table | — | Not a CLOSER stage by itself; it's the offer, which Sell should have already justified. |
| FAQ | E, weak | Present, but doesn't answer the objections a bookkeeping buyer actually has (data security, switching from Excel/QuickBooks, accountant access). |
| CTA band | — | No Reinforce. Nothing tells the reader they're making a smart, low-risk choice at the exact moment they'd click. |
...
2. **Is this you? — L.** Three lines: "Ledgerly is built for you if you're a solo founder or
   ...
   need a full-service bookkeeper (see Pilot instead)."
